...
function injectReactNative(dirPath, reset = false) {
  ...
  createViewJsFiles.forEach(function(createViewFilePath) {
    ...
    if (fs.existsSync(jsFile)) {
      if (reset) {
        console.log(`reset render.js: ${jsFile}`);
...
function reactNativeNavigationScreenString() {
  var script = `var growingPageName = null;
   ...
   if (event.id == "didAppear") {
        require('react-native').NativeModules.RNSensorsAnalyticsModule.onPageShow(growingPageName);
    }`;
  return script;
}
...
`;
  ...
  script = `${script} var pageName = $$$getActivePageName$$$(${currentStateVarName});
	...
	require('react-native').NativeModules.RNSensorsAnalyticsModule.onPageShow(pageName);}`;
  return script;
}
...
function onPressTransformer(content) {
  ...
  var injectScript =
    ...
    "this.props.onPress&&ReactNative.NativeModules.RNSensorsAnalyticsModule.trackViewClick(ReactNative.findNodeHandle(this));";
...
function createViewTransformer(content) {
  ...
  var functionBody = `var clickable = false;
		var params = ${propsName}.SAParams;
         if(${propsName}.onStartShouldSetResponder){
             clickable = true;
         }
         require('react-native').NativeModules.RNSensorsAnalyticsModule.prepareView(${tagName}, clickable, params);
                 `;
